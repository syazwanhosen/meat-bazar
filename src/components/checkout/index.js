...
export default function Checkout({ productId }) {
    ...
    const [order, setOrder] = useState({
        ...
        orderNotes: "",
        orderTime: new Date().toISOString(),  // Added orderTime field
    });
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                setOrder((prev) => ({
                    ...prev,
                    total: res.data.price * prev.quantity, // calculate total by price * quantity
                }));
            } catch (err) {
        ...
        try {
            const response = await axios.post(
                "http://localhost:5000/api/orders",
                {
                    ...
                    orderTime: new Date(order.orderTime).toISOString(), // Send ISO string
                },
                {
                    headers: {
    ...
    return (
        <div className="bg-gray-100 min-h-screen">
            ...
            <div className="relative flex justify-center items-center min-h-screen">
                ...
                />
                <div className="relative p-6 rounded-lg shadow-lg w-2/3 flex justify-between text-black bg-white bg-opacity-90">
                    {/* Left Section */}
                    <div className="w-1/2 p-4">
                        ...
                        <div className="mt-2">
                            <label className="block text-sm font-semibold">Order Time:</label>
                            <input
                                type="datetime-local"
                                name="orderTime"
                                value={order.orderTime.slice(0, 16)} // yyyy-MM-ddTHH:mm
                                onChange={handleInputChange}
                                className="w-full p-2 border rounded"
                            />
                        </div>
